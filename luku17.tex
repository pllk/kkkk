...
% Seuraavaksi käymme läpi algoritmit,
% joilla voi tarkistaa,
% onko verkko vahvasti yhtenäinen,
% sekä jakaa verkko vahvasti
% yhtenäisiin komponentteihin.
% 
% \subsection{Yhtenäisyyden tarkistus}
% 
% Vahvasti yhtenäisyyden tarkistaminen
% on helppoa, koska riittää valita
% mikä tahansa solmu $x$ verkosta
% ja tarkistaa, että solmusta $x$ pääsee
% kaikkiin verkon solmuihin
% ja vastaavasti kaikista verkon
% solmuista pääsee solmuun $x$.
% Verkko on vahvasti yhtenäinen,
% jos molemmat ehdot toteutuvat.
% 
% Kätevä tapa toteuttaa tarkistus
% on käyttää kahta syvyyshakua.
% Ensimmäinen syvyyshaku tarkistaa,
% että solmusta $x$ pääsee kaikkiin
% solmuihin verkon kaaria pitkin.
% Toinen syvyyshaku tehdään samoin,
% mutta ennen sitä kaikki kaarten suunnat
% muutetaan käänteisiksi.
% Jos molemmat syvyyshaut saavuttavat kaikki solmut,
% verkko on vahvasti yhtenäinen.
% 
% \subsection{Komponenttien etsintä}
...
Ensimmäinen syvyyshaku käy läpi
kaikki verkon solmut ja
muodostaa listan solmuista
siinä järjestyksessä kuin niiden
käsittely on päättynyt.
Solmu lisätään listalle,
kun kaikki siitä lähtevät
kaaret on käsitelty.

Seuraava kuva näyttää solmujen
käsittelyjärjestyksen esimerkkiverkossa:
\\
\begin{center}
\begin{tikzpicture}[scale=0.9,label distance=-2mm]
\node[draw, circle] (1) at (-1,1) {$7$};
...
Vahvasti yhtenäisyys liittyy myös 2SAT-ongelman ratkaisuun.
...
Tämä johtuu siitä, että jos $x_1$ on tosi,
pitäisi päteä sekä $x_3$ että $\lnot x_3$,
mikä on mahdotonta.
Toisaalta jos $x_1$ on epätosi,
