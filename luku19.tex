...
taas on Eulerin polku, jonka alku- ja loppusolmu on sama.
...
on kuitenkin Eulerin polku, jonka päätesolmut ovat
...
lisäämällä sen osaksi uusia kierroksia.
Tämä jatkuu niin kauan, kunnes kaikki kaaret kuuluvat
kierrokseen ja Eulerin kierros on valmis.

Algoritmi laajentaa kierrosta valitsemalla jonkin
kierrokseen kuuluvan solmun,
jonka kaikki kaaret eivät ole vielä mukana kierroksessa.
Algoritmi muodostaa valitusta solmusta alkaen uuden polun
...
\path[draw=red,thick,->,line width=2pt] (3) -- node[font=\small,label={[red]west:3.}] {} (1);
\end{tikzpicture}
\end{center}
...
\path[draw=red,thick,->,line width=2pt] (5) -- node[font=\small,label={[red]north:3.}] {} (6);
\path[draw=red,thick,->,line width=2pt] (6) -- node[font=\small,label={[red]north:4.}] {} (2);
\path[draw=red,thick,->,line width=2pt] (2) -- node[font=\small,label={[red]north:5.}] {} (3);
\path[draw=red,thick,->,line width=2pt] (3) -- node[font=\small,label={[red]west:6.}] {} (1);
\end{tikzpicture}
\end{center}
...
\path[draw=red,thick,->,line width=2pt] (7) -- node[font=\small,label={[red]north:7.}] {} (6);
\path[draw=red,thick,->,line width=2pt] (6) -- node[font=\small,label={[red]north:8.}] {} (2);
...
Eulerin kierroksen muodostus on mukavaa toteuttaa
...
\end{lstlisting}
Koodi pitää yllä pinoa \texttt{s},
jossa on solmuja, joiden kaikkia kaaria
ei ole vielä käsitelty.
Algoritmin päätteeksi vektori \texttt{e} sisältää
Eulerin kierrokseen kuuluvat
solmut siinä järjestyksessä,
kuin ne esiintyvät kierroksessa.
...
Myös toteutus ajassa $O(n+m)$ ilman joukkoja
...
on verkossa oleva polku, joka käy tasan kerran
jokaisessa \textit{solmussa}.
Hamiltonin kierros (\textit{Hamiltonian cycle})
taas on Hamiltonin polku,
...
\path[draw=red,thick,->,line width=2pt] (5) -- (3);
\path[draw=red,thick,->,line width=2pt] (3) -- (2);
\path[draw=red,thick,->,line width=2pt] (2) -- (1);
\end{tikzpicture}
\end{center}

Vaikka Eulerin polku ja Hamiltonin polku 
muistuttavat paljon toisiaan,
niihin liittyvät laskennalliset ongelmat
ovat tyystin erilaisia.
Toisin kuin Eulerin polun tapauksessa,
Hamiltonin polun etsimiseen ei tunneta
mitään tehokasta algoritmia, vaan kyseessä
on NP-täydellinen ongelma.

Yksinkertaisin tapa etsiä Hamiltonin polkua
on käyttää peruuttavaa hakua ja käydä läpi
kaikki vaihtoehdot muodostaa polku.
Tämä on kuitenkin hidas menetelmä,
koska pahimmillaan algoritmi voi joutua tarkistamaan
$O(n!)$ vaihtoehtoa polun muodostamiseen.

Hieman tehokkaampi tapa perustuu dynaamiseen
ohjelmointiin solmujen osa\-joukoille.
Ideana on laskea arvoja funktiolle $f(s,x)$:
onko verkossa polkua, joka käy tasan kerran
jokaisessa osajoukon $s$ solmussa
ja päättyy solmuun $x$.
Tällaisen algoritmin aikavaativuus on $O(2^n n^2)$.

Yleensä ottaen jos verkossa on paljon kaaria,
siitä on todennäköisesti helppoa löytää
jokin Hamiltonin polku.
Tähän liittyvä tulos on Diracin lause,
jonka mukaan $n$-solmuisessa verkossa
on varmasti Hamiltonin kierros,
jos jokaisen solmun aste on vähintään $n/2$.