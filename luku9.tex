...
Välikysely (\textit{range query}) kohdistuu
...
\begin{itemize}
\item \textbf{summakysely}: laske välin $[a,b]$ lukujen summa
\item \textbf{minimikysely}: etsi pienin luku välillä $[a,b]$
\item \textbf{maksimikysely}: etsi suurin luku välillä $[a,b]$
\end{itemize}
...
\subsection{Summakysely}

Summakyselyyn on mahdollista vastata tehokkaasti
muodostamalla taulukosta etukäteen summataulukko,
...
Binääri-indeksipuu\footnote{Tämä tietorakenne tunnetaan myös
nimellä Fenwick-puu (\textit{Fenwick tree}).} (\textit{binary indexed tree}) on
...
Segmenttipuu (\textit{segment tree}) on tietorakenne,
...
kiertää usein käyttämällä indeksien pakkausta.
