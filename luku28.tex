...
\section{Kulku puussa}

Tähän mennessä olemme kulkeneet segmenttipuuta
alhaalta ylöspäin lehdistä juureen.
Vaihtoehtoinen tapa toteuttaa puun käsittely
on kulkea ylhäältä alaspäin juuresta lehtiin.
...
Kun $w$ on välin $[c,d]$ pituus,
vasen puolisko kattaa välin $[c,c+w/2-1]$
...
ja puu voi myös tukea samaan aikaan
molempia muutostapoja.

\subsection{Esimerkki}

\textit{Tehtävä:} Toteuta segmenttipuu,
jonka avulla voi kasvattaa jokaista välin $[a,b]$
arvoa $x$:llä sekä laskea välin $[a,b]$ summan.

Jokaisessa puun solmussa on kaksi arvoa ($s/z$):
...
solmun arvo $z$ kasvaa $x$:llä ja kulku pysähtyy.
Jos taas väli $[c,d]$ kuuluu osittain välille $[a,b]$,
solmun arvo $s$ kasvaa $wx$:llä,
missä $w$ on välien $[a,b]$ ja $[c,d]$ yhteisen osan pituus,
...
se kasvattaa kyselyn tuloksena olevaa summaa arvolla $s+(d-c+1)z$.
Muussa tapauksessa kulku jatkuu rekursiivisesti alaspäin solmun lapsiin.

Laiskat muutokset välitetään puussa alaspäin
samalla kun puussa kuljetaan alaspäin
välin muutoksen tai summakyselyn yhteydessä.
Ideana on, että laiska muutos etenee vain
silloin, kun siihen on todellinen syy.
Useita laiskoja muutoksia voi olla
myös päällekkäin niin, että $z$
kertoo niiden summan.
...
eli välillä $[a,b]$ kohta $i$ muutos on $p(i-a)$.
...
Summan saa laskettua tehokkaasti osissa summakaavalla.
...
koska $p'(x)=p(x+c)$ on samanasteinen polynomi
kuin $p(x)$, kun $c$ on vakio.
